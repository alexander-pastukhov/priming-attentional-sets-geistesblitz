...
transformed parameters {
...

  {
    ...
    for (irow in 1:N){
      ...
      if (trial[irow] == 1) {
        obj_salience = rep_vector(1.0 / ObjN, ObjN);
      ...
      for (iObj in 1:ObjN){
        if (is_same_color_as_target[irow, iObj]) {
          obj_salience[iObj] += color_modulation[trial_attention_set[irow], participant[irow]] * obj_set_delta[1, participant[irow]] * (1 - obj_salience[iObj]);
        } else {
          obj_salience[iObj] += obj_set_delta[1, participant[irow]] * (is_target[irow, iObj] - obj_salience[iObj]);
        }
      }
