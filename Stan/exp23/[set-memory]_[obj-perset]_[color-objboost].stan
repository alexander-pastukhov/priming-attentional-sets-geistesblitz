    ...
    for (irow in 1:N){
      ...
      for (iObj in 1:ObjN){
        if (is_same_color_as_target[irow, iObj]) {
          obj_salience[trial_attention_set[irow], iObj] += color_modulation[trial_attention_set[irow], participant[irow]] * obj_set_delta[1, participant[irow]] * (1 - obj_salience[trial_attention_set[irow], iObj]);
        } else {
          obj_salience[trial_attention_set[irow], iObj] += obj_set_delta[1, participant[irow]] * (is_target[irow, iObj] - obj_salience[trial_attention_set[irow], iObj]);
        }
      }
