...
parameters {
  ...
  matrix[3, ParticipantsN] z_obj_set_color_delta;
  vector[3] mu_obj_set_color_delta;
  cholesky_factor_corr[3] l_rho_obj_set_color_delta;
  vector<lower=0>[3] sigma_obj_set_color_delta;
...
transformed parameters {
  vector[N] logMu;
  matrix[3, ParticipantsN] obj_set_color_delta = inv_logit(rep_matrix(mu_obj_set_color_delta, ParticipantsN) + diag_pre_multiply(sigma_obj_set_color_delta, l_rho_obj_set_color_delta) * z_obj_set_color_delta);

  {
    row_vector [ObjN] obj_memory;
    matrix [SetsN, ColorsN] color_memory;
    row_vector[ColorsN] color_salience;
    ...
    for (irow in 1:N){
      ...
      if (trial[irow] == 1) {
        obj_memory = rep_row_vector(1.0 / ObjN, ObjN);
        color_memory = rep_matrix(1.0 / ColorsN, SetsN, ColorsN);
        set_salience = rep_vector(1.0 / SetsN, SetsN);
      }

      // computing target inconspicuousness
      round_salience = obj_memory;
      color_salience = color_memory[trial_attention_set[irow]] / sum(color_memory[trial_attention_set[irow]]);
      for(iObj in 1:ObjN) {
        real extra_boost = presence[trial_attention_set[irow], participant[irow]] * is_round_object[irow, iObj] + 
                           color_modulation[trial_attention_set[irow], participant[irow]] * color_salience[object_color[irow, iObj]];
      ...
      for (iObj in 1:ObjN) obj_memory[iObj] += obj_set_color_delta[1, participant[irow]] * (is_target[irow, iObj] - obj_memory[iObj]);
      
      // adjusting color memory
      for(iColor in 1:ColorsN) color_memory[trial_attention_set[irow], iColor] += obj_set_color_delta[3, participant[irow]] * (is_target_color[irow, iColor] - color_memory[trial_attention_set[irow], iColor]);

      // attention set saliency changes (the other set is complimentary)
      set_salience[1] += obj_set_color_delta[2, participant[irow]] * (is_direct_attention_set[irow] - set_salience[1]);
...
model {
  ...
  mu_obj_set_color_delta ~ normal(0, 1);       // regularizing prior with mode at 0.5
  to_vector(z_obj_set_color_delta) ~ normal(0, 1);
  l_rho_obj_set_color_delta ~ lkj_corr_cholesky(2);
  sigma_obj_set_color_delta ~ exponential(10);
