    ...
    for (irow in 1:N){
      ...
      for (iObj in 1:ObjN){
        if (is_same_color_as_target[irow, iObj]) {
          obj_memory[iObj] += color_modulation[trial_attention_set[irow], participant[irow]] * obj_delta[participant[irow]] * (1 - obj_memory[iObj]);
        } else {
          obj_memory[iObj] += obj_delta[participant[irow]] * (is_target[irow, iObj] - obj_memory[iObj]);
        }
      }
    }
  }
