...
parameters {
  ...
  matrix[2, ParticipantsN] z_obj_color_delta;
  vector[2] mu_obj_color_delta;
  cholesky_factor_corr[2] l_rho_obj_color_delta;
  vector<lower=0>[2] sigma_obj_color_delta;
...
transformed parameters {
  vector[N] logMu;
  matrix[2, ParticipantsN] obj_color_delta = inv_logit(rep_matrix(mu_obj_color_delta, ParticipantsN) + diag_pre_multiply(sigma_obj_color_delta, l_rho_obj_color_delta) * z_obj_color_delta);

  {
    ...
    for (irow in 1:N){
      ...
      if (trial[irow] == 1) {
        obj_memory = rep_vector(1.0 / ObjN, ObjN);
      ...
      // compute logMu using target saliency
      ...
      for (iObj in 1:ObjN) obj_memory[iObj] += obj_color_delta[1, participant[irow]] * (is_target[irow, iObj] - obj_memory[iObj]);

      // adjusting color memory
      for(iColor in 1:ColorsN) color_memory[trial_attention_set[irow], iColor] += obj_color_delta[2, participant[irow]] * (is_target_color[irow, iColor] - color_memory[trial_attention_set[irow], iColor]);
    }
  }
}
...
model {
  ...
  mu_obj_color_delta ~ normal(0, 1);       // regularizing prior with mode at 0.5
  to_vector(z_obj_color_delta) ~ normal(0, 1);
  l_rho_obj_color_delta ~ lkj_corr_cholesky(2);
  sigma_obj_color_delta ~ exponential(10);
